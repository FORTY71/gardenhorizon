...
-- ==========================
-- REWORKED AUTO BUY LOGIC 
-- ==========================

-- Cek stok lebih kebal error (Jika remote berubah jadi Event, dia tidak akan macet)
local function getShopStockAmount(remote, shopType, itemName)
    if not remote then return math.huge end 
    
    if remote:IsA("RemoteFunction") then
        local ok, data = pcall(function() return remote:InvokeServer(shopType) end)
        if ok and type(data) == "table" and type(data.Items) == "table" then
            for k, v in pairs(data.Items) do
                if tostring(k):lower() == tostring(itemName):lower() then
                    return tonumber(v.Amount) or 0
                end
            end
            return 0 
        end
    ...
    -- Kalau gagal ambil, kita asumsikan stoknya Infinity (agar loop buy tetap mencoba mengeksekusi)
    return math.huge 
end

local function getSeedStockAmount(seedName)
    return getShopStockAmount(GetShopDataRemote, "SeedShop", seedName)
end

local function getGearStockAmount(gearName)
    return getShopStockAmount(GetShopDataRemote, "GearShop", gearName)
end
...
local function tryBuySelectedSeed(silent, forcedSeedName)
    ...
    if not PurchaseShopItemRemote then 
        if not silent then Notify("Purchase remote not found.", 3) end
        return false 
    end
    if not seedName then 
        if not silent then Notify("Select a seed first.", 3) end
        return false 
    end

    if Settings.CheckSeedStockBeforeBuy then
        local stockAmount = getSeedStockAmount(seedName)
        if stockAmount <= 0 then 
            if not silent then Notify(seedName .. " is out of stock.", 3) end
            return false 
        end
    end
    ...
    if not canAffordPrice(seedPrice) then 
        if not silent then Notify("Not enough shillings.", 3) end
        return false 
    end

    local root = getCharacterRoot()
    local originalPos = root and root.Position or nil
    local didTeleport = false

    -- Optimasi Teleport: Jangan teleport kalau sudah dekat NPC
    if Settings.TeleportToShopOnBuy and root and Settings.SeedShopNpcPosition then
        if not isNearNpc(root.Position, Settings.SeedShopNpcPosition, 10, 15) then
            local reached = teleportRootAndWait(root, Settings.SeedShopNpcPosition, 1.2, 2.5, 10, 5)
            if not reached then 
                if not silent then Notify("Could not reach seed shop NPC.", 3) end
                return false 
            end
            didTeleport = true
            task.wait(0.2)
    ...
    -- Menyesuaikan apakah remote itu Function atau Event, dan menambahkan argument "1" (Amount)
    local ok, result, reason = pcall(function() 
        if PurchaseShopItemRemote:IsA("RemoteFunction") then
            return PurchaseShopItemRemote:InvokeServer("SeedShop", seedName, 1) 
        else
            PurchaseShopItemRemote:FireServer("SeedShop", seedName, 1)
            return true, "Fired"
        end
    end)
    ...
    if ok and result then
        ...
        if not silent then Notify("Purchase failed: " .. tostring(reason or "Unknown Error"), 3) end
        return false
    end
end
...
local function tryBuySelectedGear(silent, forcedGearName)
    ...
    if not gearName then 
        if not silent then Notify("Select a gear first.", 3) end
        return false 
    end
    
    if Settings.CheckGearStockBeforeBuy then
        local stockAmount = getGearStockAmount(gearName)
        if stockAmount <= 0 then 
            if not silent then Notify(gearName .. " is out of stock.", 3) end
            return false 
        end
    end
    ...
    -- Optimasi Teleport: Jangan teleport kalau sudah dekat NPC
    if Settings.TeleportToGearShopOnBuy and root and Settings.GearShopNpcPosition then
        if not isNearNpc(root.Position, Settings.GearShopNpcPosition, 10, 15) then
            local reached = teleportRootAndWait(root, Settings.GearShopNpcPosition, 1.2, 2.5, 10, 5)
            if not reached then 
                if not silent then Notify("Could not reach gear shop NPC.", 3) end
                return false 
            end
            didTeleport = true
            task.wait(0.2)
    ...
    -- Menyesuaikan Function/Event
    local ok, result, reason = pcall(function() 
        if PurchaseShopItemRemote:IsA("RemoteFunction") then
            return PurchaseShopItemRemote:InvokeServer("GearShop", gearName, 1) 
        else
            PurchaseShopItemRemote:FireServer("GearShop", gearName, 1)
            return true, "Fired"
        end
    end)
...
print("[Garden Horizons] System Loaded (Auto Buy Fixed!).")

...
-- Remotes (Fruit Harvest)
local HarvestFruitRemote = nil
pcall(function()
    HarvestFruitRemote = ReplicatedStorage:WaitForChild("Remotes", 5):WaitForChild("HarvestFruit", 5)
end)
...
local Settings = {
    ...
    -- Fruit Harvest Settings
    AutoHarvestFruit = false,
    FruitCategory = "All",
    FruitDelay = 0.05
}
...
MainTab:AddLabel("--- Auto Harvest (Prompt) ---")
...
-- FITUR BARU: AUTO FRUIT HARVEST (Remote)
MainTab:AddLabel("--- Auto Fruit Harvest (Remote) ---")
MainTab:AddToggle("Enable Auto Fruit Harvest", false, function(value) Settings.AutoHarvestFruit = value end)
MainTab:AddDropdown("Fruit Category", {"All", "Ripe", "Lush", "Unripe"}, false, function(value) 
    local selected = pickFirstSelectedValue(value, nil)
    if selected then Settings.FruitCategory = tostring(selected) end
end)
MainTab:AddSlider("Fruit Harvest Delay (s)", 0.05, 1.0, Settings.FruitDelay, function(value) Settings.FruitDelay = value end)
...
local function tryBuySelectedSeed(silent, forcedSeedName)
    ...
    if Settings.CheckSeedStockBeforeBuy then
        ...
        if stockAmount == nil then
            if not silent then Notify(stockErr or "Could not check stock.", 3) end
            return false
        end
    ...
    if not canAffordPrice(seedPrice) then
        if not silent then Notify("Not enough shillings to buy " .. seedName .. ".", 3) end
        return false
    end
    ...
    if not ok then
        if not silent then Notify("Purchase failed (invoke error).", 3) end
        return false
    end
...
local function tryBuySelectedGear(silent, forcedGearName)
    ...
    if not canAffordPrice(gearPrice) then
        if not silent then Notify("Not enough shillings to buy " .. gearName .. ".", 3) end
        return false
    end
        ...
        if d:IsA("ProximityPrompt") and d.Parent and d.Enabled and (not isIgnoredSignPrompt(d)) then
            ...
            if pos then
                table.insert(entries, {
                    Prompt = d, Pos = pos, OwnerUserId = ownerUserId, IsFavorited = isFavorited,
                })
            end
        end
    end
...
local function fireHarvest(plantUuid, anchorIndex)
    if not HarvestFruitRemote then return end
    pcall(function()
        HarvestFruitRemote:FireServer({
            [1] = {
...
-- Loop 1: Main Features (Proximity, Plant, Buy, Sell, Quests)
task.spawn(function()
...
-- Loop 2: Auto Fruit Harvest (Remotes)
task.spawn(function()
    while true do
        if Settings.AutoHarvestFruit and HarvestFruitRemote then
            local clientPlants = findClientPlants()
            if clientPlants then
                for _, plant in ipairs(clientPlants:GetChildren()) do
                    if not Settings.AutoHarvestFruit then break end
                    ...
                    for _, fruit in ipairs(plant:GetChildren()) do
                        ...
                        if not fruitMatchesCategory(fruit, Settings.FruitCategory) then continue end

                        local anchor = tonumber(
                            fruit:GetAttribute("GrowthAnchorIndex") or fruit:GetAttribute("AnchorIndex") or 1
                        ) or 1
...
print("[Garden Horizons] UI & AutoHarvest Loaded.")
